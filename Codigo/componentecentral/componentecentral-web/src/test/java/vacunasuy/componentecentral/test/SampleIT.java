package vacunasuy.componentecentral.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "componentecentral-ear.ear");

        // Current directory is the root of the "componentecentral-web" project. Go up one level
        // and enter the "componentecentral-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../componentecentral-ejb/target/componentecentral-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "componentecentral-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/componentecentral-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "componentecentral-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("vacunasuy.componentecentral.test");
