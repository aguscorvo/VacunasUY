package vacunasuy.nodosperifericos.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "nodosperifericos-ear.ear");

        // Current directory is the root of the "nodosperifericos-web" project. Go up one level
        // and enter the "nodosperifericos-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../nodosperifericos-ejb/target/nodosperifericos-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "nodosperifericos-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/nodosperifericos-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "nodosperifericos-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("vacunasuy.nodosperifericos.test");
